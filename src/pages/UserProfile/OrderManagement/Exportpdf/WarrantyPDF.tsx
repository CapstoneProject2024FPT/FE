...
const WarrantyPDFDocument = ({ order, product }: { order: OrderProps, product: ProductProps }) => (
  <Document>
    <Page style={styles.page}>
      ...
      <View style={styles.section}>
        ...
        <Text>Ngày mua: {formatDateFunc.formatDate(order.createDate) || ''}</Text>
...
const WarrantyPDF = ({ order, product }: { order: OrderProps, product: ProductProps }) => {
  const [pdfUrl, setPdfUrl] = useState<string | null>(null);

  useEffect(() => {
    const generatePdf = async () => {
      const blob = await pdf(<WarrantyPDFDocument order={order} product={product} />).toBlob();
  ...
  return (
    ...
    <Button variant='contained' onClick={handleOpenPdf}>Xem phiếu bảo hành</Button>
  );
};
